...
export function AvgPowerCard({ avgPowerIn, avgPowerOut, snapshots }: Props) {
  ...
  return (
    <div className="bg-white border border-border-subtle rounded-[10px] p-4 hover:border-border-card transition-colors duration-150">
      <div className="font-mono text-[8px] tracking-[.1em] uppercase text-ink3 mb-[5px]">Avg power</div>
      <div className="flex items-baseline gap-2.5 mb-2">
        <div className="flex flex-col">
          <span className="font-mono text-[19px] font-normal tracking-tight leading-none">{fmtDec(avgPowerIn, 2)}</span>
          <span className="font-mono text-[7.5px] text-ink3 tracking-[.07em] uppercase mt-[3px]">kW in</span>
        </div>
        <span className="text-[13px] text-ink4 self-center mb-[7px]">→</span>
        <div className="flex flex-col">
          <span className="font-mono text-[19px] font-normal tracking-tight leading-none text-primary">{fmtDec(avgPowerOut, 2)}</span>
          <span className="font-mono text-[7.5px] text-ink3 tracking-[.07em] uppercase mt-[3px]">kW out</span>
        </div>
      </div>
