...
public class HeatPumpAiService : IHeatPumpAiService
{
    ...
    private static async Task<PeriodStats> GetPeriodStatsAsync(CosyDbContext db, string deviceId, DateTime from, DateTime to)
    {
        ...
        return new PeriodStats
        {
            ...
            TotalEnergyInKwh = withPower.Count > 0 ? withPower.Sum(s => (double)s.PowerInputKilowatt!.Value) * Constants.SnapshotIntervalHours : null,
            TotalHeatOutKwh = withHeat.Count > 0 ? withHeat.Sum(s => (double)s.HeatOutputKilowatt!.Value) * Constants.SnapshotIntervalHours : null,
